...
\item In Python, {\tt multiprocessing.RawArray(ctypes.c\_double, N)} creates an array of $N$ doubles that can be shared among multiple processes (if you fork them as instances of {\tt multiprocessing.Process}). Cast that memory as Numpy in the shared processes to do work in parallel.
